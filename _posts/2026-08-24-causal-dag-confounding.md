---
title: "인과추론 입문 2: DAG로 교란 찾기와 표준화 조정"
date: 2026-08-24 09:00:00 +0900
categories: [기반 지식, 학습과 모델링]
tags: [인과추론, causal-DAG, 교란, 뒷문경로, 표준화, Python]
description: "관찰 데이터에서 교란이 평균 차이를 왜곡하는 이유를 인과 DAG로 이해하고, 뒷문 경로를 막는 층화·표준화 조정을 손계산과 Python으로 배웁니다."
reading_time: 13
series: "처음 시작하는 인과추론"
level: "입문"
math: true
...
지난 글에서는 잠재적 결과 $Y(1)$과 $Y(0)$, 평균처리효과(ATE), 무작위배정을 배웠다. 아직 읽지 않았다면 [인과추론 입문 1: 상관관계와 잠재적 결과]({{ '/posts/causal-inference-roadmap/' | relative_url }})부터 읽는 것을 권한다.
...
다음 글에서는 DAG의 경로가 언제 열리고 닫히는지 더 정확히 배운다. 특히 두 화살표가 만나는 **콜라이더(collider)**를 통제하면 없던 연관성이 왜 생기는지 작은 표와 Python으로 확인할 예정이다.
